...
function push(directory, expected) {
  ...
  if (expected && head !== expected)
    throw new Error(`Submodule differs from committed pointer: ${directory}`);
  ...
  if (!branch) {
    ...
    if (!published)
      throw new Error(
        `Check out a branch before publishing detached commits: ${directory}`,
      );
  }
  for (const entry of git(directory, 'ls-tree', '-rz', 'HEAD').split('\0')) {
    ...
    if (!existsSync(resolve(child, '.git')))
      throw new Error(`Initialize submodule first: ${path}`);
