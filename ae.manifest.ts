...
export default {
  ...
  design: {
    ...
    infrastructure:
      'Cloudflare; first launch configuration and acceptance: cloudflare/deployment.manifest.ts.',
    deployment:
      'Cloudflare Workers Builds checks pinned private submodules and builds both Workers. Root migration and deploy commands follow cloudflare/deployment.manifest.ts; first complete automated release remains pending.',
    prototypes: [
      'public-landing',
      'writing-studio',
      'dashboard',
    ...
    direction:
      'Approved workspace composition and sun/moon themes follow AE Design. Public page composition stays in web/web.manifest.ts.',
    landing:
      'Public composition belongs to web/web.manifest.ts. Visual language, brand, tokens, and shared behavior belong to design/design.manifest.ts.',
    entry:
      'Public site first. Separate public navigation and workspace sidebar. Owner workspace opens on Dashboard.',
    guest:
      'First deployment protects guest and owner workspaces with the single owner passkey session. Public guest access is deferred.',
  },
  risks: [
    {
      ...
      owner: 'amazingefren',
      scenario:
        'A malicious or compromised direct or transitive npm package executes during install, build, or runtime.',
      consequence:
        'Developer or CI credentials stolen, private source exposed, or malicious code shipped to visitors.',
      status: 'open',
      inherent: {
        likelihood: 3,
        impact: 5,
        rationale:
          'Initial qualitative estimate; dependencies execute code with the permissions of their environment.',
      },
      residual: null,
      treatment: {
        strategy: 'reduce',
        action:
          'Minimize dependencies; verify package identity; review direct and transitive lockfile changes; use locked CI installs; deny install scripts by default with reviewed exceptions; isolate builds from private material and deployment credentials; scan advisories and provenance where available. These signals do not prove code safe. On compromise, stop releases, identify affected artifacts, revoke exposed credentials, and rebuild from reviewed inputs in a clean environment.',
        status: 'proposed',
        implementations: [],
        evidence: [],
      },
      ...
      reviewTrigger:
        'Dependency or lockfile changes, new install scripts, security advisories, and before CI or deployment receives secrets.',
    },
    {
      id: 'ae.provider-loss',
      owner: 'amazingefren',
      ...
      treatment: {
        strategy: 'reduce',
        action:
          'Keep encrypted backups outside the provider account; test D1/R2 restoration and an alternate runtime; retain independent domain control.',
        status: 'proposed',
        implementations: [],
        evidence: [],
      },
    ...
    },
    {
      ...
      owner: 'amazingefren',
      scenario:
        'Backups are missing, corrupt, or share the failed provider account.',
      ...
      treatment: {
        strategy: 'reduce',
        action:
          'Define recovery time and data-loss targets; automate independent encrypted exports and verify restores.',
        status: 'proposed',
        implementations: [],
        evidence: [],
      },
  ...
  frontend: {
    public: 'RedwoodSDK SSR',
    interactive: 'React',
    language: 'TypeScript',
    styling: 'scoped CSS and CSS Modules',
    runtime: 'Cloudflare Workers',
    workspace: 'npm',
  },
  authoring: {
    system: 'studio',
    runtime: 'ae-studio-engine',
  ...
  contentStorage: {
    sourceOfTruth: 'D1',
    files: 'R2',
    gitContent: 'fixtures-and-approved-exports-only',
  ...
  access: {
    ...
    invariants: [
      ...
      'same-access-rules',
      'no-private-data-in-public-exports',
      'source-provenance',
    ],
  },
  patterns: [
  ...
  verification: {
    approach:
      'Implement first; verify declared obligations with independent expectations.',
  ...
  rules: 'AGENTS.md',
  formatting: 'scripts/format/format.manifest.ts',
  ...
  privateMaterial: {
    ...
    manifest: 'ae-workbench.manifest.ts',
    requiredForPublicBuild: false,
  },
  privateEngines: [
    'ae-resume-engine',
    'ae-publishing-engine',
    'ae-system-engine',
    'ae-studio-engine',
    'ae-eval-engine',
  ],
