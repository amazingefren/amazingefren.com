export function installObservationMotion(
  document: Document,
  ...
  document.documentElement.setAttribute(
    'data-observation-birds',
    settled ? 'settled' : 'entering',
  );
  function sync() {
    ...
    root.setAttribute(
      'data-observation-birds',
      dark ? 'waiting' : settled ? 'settled' : 'entering',
    );
    if (!scene) {
      root.removeAttribute('data-observation-motion');
      return;
    }
    root.setAttribute(
      'data-observation-motion',
  ...
  function finish(event: Event) {
    if (
      isDark() ||
      (event as AnimationEvent).animationName !== 'ae-observation-trails'
    )
      return;
    settled = true;
    sync();
