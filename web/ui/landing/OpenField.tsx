    ...
    <>
      <div className="open-field-background" aria-hidden="true">
        ...
        >
          <g className="observation-birds">
            ...
            >
              {Array.from({ length: 17 }, (_, i) => (
                <path key={`v${i}`} d={`M${i * 90} 0V1000`} />
              ))}
              {Array.from({ length: 12 }, (_, i) => (
                <path key={`h${i}`} d={`M0 ${i * 90}H1440`} />
              ))}
            </g>
            <g className="observation-mobile-scene">
              <g
                className="observation-trails"
                fill="none"
                stroke="var(--ae-muted)"
                strokeWidth="1"
                strokeDasharray="3 9"
              ...
              {birds.map((bird, index) => (
                <g key={index} transform={`translate(${bird.x} ${bird.y})`}>
                  <g
                    className={`observation-flight observation-flight-${index}`}
                    style={{
                      animationDelay: `${bird.enter}s`,
                      animationDuration: `${bird.duration}s`,
                    }}
                  >
                    <g
                      fill="var(--ae-body-copy)"
                      opacity={bird.opacity}
                      transform={`rotate(${bird.angle}) scale(${bird.scale})`}
                    >
                      <path d={birdShapes[bird.shape]} />
                    </g>
                    {index === 3 && (
                      <g
                        className="observation-lock"
                        style={{
                          animationDelay: `${bird.enter + bird.duration * 0.72}s`,
                          animationDuration: `${bird.duration * 0.28}s`,
                        }}
                        fill="none"
                        stroke="var(--ae-theme-accent)"
                        strokeWidth={index === 3 ? 1.8 : 1.2}
                        opacity={0.48 + (index % 3) * 0.2}
                      >
          ...
          <g className="observation-galaxy" opacity=".45">
            <image
      ...
      <section className="open-field-intro" aria-labelledby="home-title">
        <h1 id="home-title">
          My workbench
          <br />
          <em>for observability</em>
          <br />
          in the AI era.
        </h1>
        <p>
          A place to explore, experiment,
          <br />
          and share what I notice.
        </p>
        <div className="open-field-actions">
          <ActionLink variant="primary" href="/readings">
            Explore the readings
          </ActionLink>
          <ActionLink href="/about">About me</ActionLink>
        </div>
      </section>
    </>
  );
