    ...
    workers: [
      {
        ...
        d1Databases: ['AUTH_DB'],
        bindings: { AUTH_ORIGIN: origin, AUTH_BOOTSTRAP_TOKEN: secret },
...
try {
  ...
  const sql = await readFile(
    new URL('../../../auth/migrations/0001_auth.sql', import.meta.url),
    'utf8',
  );
  ...
  for (const path of [
    ...
    assert.equal((await send(path)).status, 303, path);
  assert.equal(
    (
      await send(
  ...
  const options = await send('/api/auth/registration-options', {
    bootstrapToken: secret,
  });
  assert.equal(options.status, 200);
  const challenge = (await options.json()).challenge;
  const verify = await send('/api/auth/registration-verify', {
    response: device.registration(challenge),
  });
  assert.equal(verify.status, 200);
  assert.match(
    verify.headers
      .getSetCookie()
      .find((x) => x.startsWith('__Host-ae-session=')),
  ...
  assert.equal(
    (await send('/api/auth/registration-options', { bootstrapToken: secret }))
      .status,
    403,
  );
  const authOptions = await send('/api/auth/authentication-options', {});
  const authResponse = device.authentication(
    (await authOptions.json()).challenge,
  );
  assert.equal(
    (await send('/api/auth/authentication-verify', { response: authResponse }))
      .status,
    200,
  );
  assert.equal((await send('/auth/me?__rsc_action_id=forged', {})).status, 403);
  console.log(
    `Built Worker launch integration passed: ${requests} HTTP requests; real WebAuthn registration/login, D1 sessions, logout, closed bootstrap, and route protection.`,
  );
} finally {
