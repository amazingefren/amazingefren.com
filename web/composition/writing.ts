  ...
  json(
    {
      ...
      error: {
        code: 'unavailable',
        message: 'Writing storage is unavailable.',
      },
    },
    503,
  );

export function createWritingGateway(
  ...
  return {
    async operation(request: Request): Promise<Response> {
      ...
      if (
        ...
        command.value.operation !== operation ||
        operation === 'studio.writing.reset'
      )
        return json(
          {
            ok: false,
      ...
      try {
        const response = await service.fetch(
          new Request(
            'https://workspace-owner.internal/api/writing/operation',
            {
              method: 'POST',
              headers: {
        ...
        return json(
          result.value,
          result.value.ok ? 200 : status(result.value.error.code),
        );
      } catch {
        return unavailable();
      }
    ...
    async privateAsset(request: Request): Promise<Response> {
      ...
      if (!/^\/api\/writing\/assets\/[a-zA-Z0-9_-]{1,120}$/.test(path))
        return new Response(null, { status: 404 });
      try {
        return safeAsset(
          await service.fetch(
            new Request(`https://workspace-owner.internal${path}`, {
              headers: { cookie: assertion },
            }),
          ),
        );
      } catch {
    ...
    async publicAsset(request: Request): Promise<Response> {
      const path = new URL(request.url).pathname;
      if (
        !/^\/api\/publications\/assets\/[a-zA-Z0-9_-]{1,120}\/[a-zA-Z0-9_-]{1,120}$/.test(
          path,
        )
      )
        return new Response(null, { status: 404 });
    ...
    async read(slug?: string): Promise<Result<Snapshot[]>> {
      if (slug !== undefined && !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug))
        return {
          ok: false,
          error: { code: 'missing', message: 'Publication not found.' },
        };
      if (!service)
        return {
          ok: false,
          error: {
            code: 'unavailable',
            message: 'Publications are unavailable.',
          },
        };
      try {
        const response = await service.fetch(
          new Request(
            `https://workspace-owner.internal/api/publications${slug ? `/${encodeURIComponent(slug)}` : ''}`,
          ),
        );
        const result = await boundedJson(response, responseLimit);
        if (
          ...
          Array.isArray(result.value.value) &&
          result.value.value.every(isPublicSnapshot)
        )
...
function failure(value: unknown): value is {
  ok: false;
  error: {
...
function writingResult(value: unknown): value is Result<StudioState> {
  return (
    failure(value) ||
    (record(value) &&
      value.ok === true &&
      isStudioState(value.value) &&
...
function json(value: unknown, code: number) {
...
export function isPublicSnapshot(value: unknown): value is Snapshot {
  return (
    ...
    value.assets.every(
      (asset) =>
        /^[a-zA-Z0-9_-]{1,120}$/.test(asset.id) &&
        asset.dataUrl === `/api/publications/assets/${value.id}/${asset.id}`,
    )
  );
}
