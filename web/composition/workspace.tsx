...
import type { CatalogEntry, WorkspacePage } from '../../workspace/contracts/index.ts';
...
function publicCatalog(): CatalogEntry[] {
  return systems.filter(system => system.visibility === 'public').map(system => ({
    id: system.id, name: system.name, purpose: system.purpose, status: system.status,
    capabilities: [...system.capabilities], dependencies: [...system.dependencies], contracts: [...system.contracts],
    operations: system.operations.map(operation => ({ id: operation.id, access: operation.access.kind, permissions: operation.access.kind === 'public' ? [] : [...operation.access.permissions], bindings: operation.bindings.map(binding => {
      const surface = binding.surface;
      const label = surface.kind === 'http' ? `${surface.method} ${surface.path}` : surface.kind === 'mcp-tool' ? surface.name : surface.kind === 'mcp-resource' ? surface.uriTemplate : surface.kind === 'cli' ? surface.command : surface.kind === 'mirror' ? surface.network : surface.path;
...
export function createOwnerWorkspaceRoute(gateway: OwnerWorkspaceGateway) {
  ...
  const requested = path.split('/')[2] || 'dashboard';
  if (!workspacePages.includes(requested as WorkspacePage)) { response.status = 404; return <main className="ae-workspace ws-main"><h1>Page not found</h1><a href="/workspace/dashboard">Open Dashboard</a></main>; }
  return <WorkspaceApp initialPage={requested as WorkspacePage} catalog={publicCatalog()} audience="owner" initialState={result.value} />;
  };
}
