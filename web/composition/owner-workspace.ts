...
export function createOwnerWorkspaceGateway(service: OwnerWorkspaceService | undefined): OwnerWorkspaceGateway {
  return {
    async read(request) {
      return invoke(service, request, JSON.stringify({ operation: 'workspace.read', input: {} }));
    },
    async operation(request) {
      ...
      return result.ok ? json(result, 200) : errorResponse(result.error.code, result.error.message, status(result.error.code));
    }
  };
}
...
async function commandBody(request: Request): Promise<{ ok: true; value: string } | { ok: false; error: { code: 'invalid'; message: string } }> {
  const declaredLength = request.headers.get('content-length');
  if (declaredLength !== null && (!/^\d+$/.test(declaredLength) || Number(declaredLength) > MAX_COMMAND_BYTES)) return { ok: false, error: { code: 'invalid', message: 'Workspace command exceeds the maximum size' } };
  if (!request.body) return { ok: false, error: { code: 'invalid', message: 'Workspace command body is unavailable' } };
  ...
  try {
    while (true) {
      ...
      if (length > MAX_COMMAND_BYTES) {
        await reader.cancel();
        return { ok: false, error: { code: 'invalid', message: 'Workspace command exceeds the maximum size' } };
      }
      chunks.push(chunk.value);
    }
  } catch {
    return { ok: false, error: { code: 'invalid', message: 'Workspace command body is unavailable' } };
  } finally {
    reader.releaseLock();
  }
  ...
  try {
    ...
    return parsed.ok && parsed.value.operation !== 'workspace.reset' ? { ok: true, value: body } : { ok: false, error: { code: 'invalid', message: parsed.ok ? 'Workspace reset is available only to guest sessions' : parsed.error.message } };
  } catch {
    return { ok: false, error: { code: 'invalid', message: 'Workspace command must be JSON' } };
  }
}
...
function json(result: WorkspaceResult<WorkspaceState>, code: number): Response {
  return new Response(JSON.stringify(result), { status: code, headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' } });
}

function errorResponse(code: 'invalid' | 'denied' | 'missing' | 'conflict' | 'unavailable', message: string, responseStatus: number, extra: Record<string, string> = {}): Response {
  return new Response(JSON.stringify({ ok: false, error: { code, message } }), { status: responseStatus, headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store', ...extra } });
}
