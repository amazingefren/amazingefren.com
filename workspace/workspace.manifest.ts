...
const commandContract =
  'workspace/operations/contracts/workspace-command.schema.json';
const ownerCommandContract =
  'workspace/operations/contracts/workspace-owner-command.schema.json';
const stateContract =
  'workspace/operations/contracts/workspace-state.schema.json';
const errorContract =
  'workspace/operations/contracts/workspace-error.schema.json';
...
const operations = [
  ...
  [
    'restore-document',
    'POST',
    'Restore a synthetic workspace document revision.',
  ],
...
function operation(
  name: (typeof operations)[number][0],
  method: 'GET' | 'POST',
  description: string,
): Operation {
  ...
  return {
    ...
    bindings: [
      ...
      },
      {
        ...
        surface: {
          kind: 'mcp-tool',
          name: `workspace_${name.replaceAll('-', '_')}`,
          description,
          readOnly: name === 'read',
        },
        scope: 'required',
    ...
    verification: [
      {
        ...
        category: 'contract',
        expectation:
          'The command parser rejects unknown fields, unsupported operations, and invalid bounded values.',
      ...
      },
      {
        ...
        category: 'behavior',
        expectation:
          'The command returns a bounded synthetic state and keeps published snapshots separate from later drafts.',
        tests: operationTests,
      },
      {
        id: `${id}.failure`,
        category: 'failure',
        expectation:
          'Missing records, revision conflicts, and unavailable dependencies return typed errors without partial writes.',
        tests: operationTests,
      },
      {
        id: `${id}.integration`,
        ...
        expectation:
          'HTTP, MCP, and CLI delegate to the same injected workspace operation boundary.',
        tests: transportTests,
      },
    ],
  };
...
const syntheticOperations = operations.map(([name, method, description]) =>
...
const ownerExecuteOperation = {
  ...
  verification: [
    {
      id: 'workspace.owner-execute.access',
      category: 'access',
      expectation:
        'The public gateway forwards an passkey session only to a configured private service. The service authorizes each declared command before storage reads.',
      tests: ['workspace/tests/adapters/owner-gateway.test.ts'],
    },
  ],
    ...
    page === 'dashboard'
      ...
      : page === 'documents'
        ? [
            ...
            'workspace.restore-document',
          ]
        : page === 'tasks'
          ? ['workspace.create-task', 'workspace.complete-task']
          : page === 'experiments'
            ? ['workspace.create-experiment', 'workspace.update-experiment']
            : page === 'relationships'
              ? ['workspace.link-documents', 'workspace.unlink-documents']
              : page === 'publishing'
                ? ['workspace.publish', 'workspace.withdraw']
                : page === 'systems'
                  ? ['workspace.reset']
                  : [];
...
export default {
  ...
  views: [
    ...pageNames.map((page) => ({
      ...
      verification: [
        {
          expectation:
            'Guest route reads only its session-isolated synthetic WorkspacePort state and fails closed when storage is unavailable.',
          tests: ['workspace/tests/adapters/guest-browser.test.ts'],
        },
      ],
    })),
    ...pageNames.map((page) => ({
      ...
      verification: [
        {
          expectation:
            'Owner route forwards the passkey session to a configured private service and renders only after its authorized workspace read succeeds.',
          tests: ['workspace/tests/adapters/owner-gateway.test.ts'],
        },
      ],
    })),
  ...
  capabilityPaths: {
    ...
    'guest-boundary': 'workspace/adapters',
    'owner-boundary': 'workspace/ports',
    transports: 'workspace/adapters',
  },
  structure: {
  ...
  entrypoints: [
    'workspace/domain/index.ts',
    'workspace/operations/index.ts',
    'workspace/ports/index.ts',
    'workspace/adapters/index.ts',
    'workspace/adapters/guest-browser.ts',
    'workspace/adapters/owner-server.ts',
  ],
