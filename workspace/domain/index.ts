...
export function isWorkspaceState(value: unknown): value is WorkspaceState {
  ...
  const valid =
    ...
    value.tasks.every(
      (item) =>
        isRecord(item) &&
        hasOnly(item, ['id', 'title', 'done', 'documentId']) &&
        validId(item.id) &&
        validText(item.title, MAX_TITLE_LENGTH) &&
        typeof item.done === 'boolean' &&
        (item.documentId === null || validId(item.documentId)),
    ) &&
    ...
    value.experiments.every(
      (item) =>
        ...
        validId(item.id) &&
        validText(item.title, MAX_TITLE_LENGTH) &&
        validText(item.hypothesis, MAX_HYPOTHESIS_LENGTH) &&
        validExperimentStatus(item.status) &&
        validBoundedText(item.observations, MAX_OBSERVATIONS_LENGTH),
    ) &&
    ...
    value.publications.every(
      (item) =>
        ...
        validId(item.documentId) &&
        validText(item.title, MAX_TITLE_LENGTH) &&
        validBoundedText(item.body, MAX_BODY_LENGTH) &&
        validRevision(item.revision) &&
        validTimestamp(item.publishedAt),
    ) &&
    ...
    value.activity.every(
      (item) =>
        isRecord(item) &&
        hasOnly(item, ['id', 'label', 'at']) &&
        validId(item.id) &&
        validText(item.label, MAX_LABEL_LENGTH + 128) &&
...
function isDocument(value: unknown): value is WorkspaceDocument {
  return (
    ...
    value.revisions.every(
      (item) =>
        isRecord(item) &&
        hasOnly(item, ['revision', 'title', 'body', 'savedAt']) &&
        validRevision(item.revision) &&
        validText(item.title, MAX_TITLE_LENGTH) &&
        validBoundedText(item.body, MAX_BODY_LENGTH) &&
...
function validExperimentStatus(
  value: unknown,
