...
export function createOwnerWorkspacePort(
  ...
  return {
    async execute(command): Promise<WorkspaceResult<WorkspaceState>> {
      ...
      if (parsed.value.operation === 'workspace.reset')
        return denied('Workspace reset is available only to guest sessions');
      let identity;
      try {
      ...
      if (
        ...
        identity.kind !== 'owner' ||
        !identity.ownerId ||
        identity.ownerId !== identity.subjectId
      )
        return denied('An authenticated owner is required');
      let authorization;
      try {
        authorization = await dependencies.authorization.authorize({
          operation: parsed.value.operation,
          permission: workspacePermission(parsed.value.operation),
          identity,
      ...
      if (!authorization.allowed || authorization.ownerId !== identity.ownerId)
        return denied(
          authorization.allowed
            ? 'Authorized owner does not match caller'
      ...
      if (state.synthetic)
        return unavailable('Owner workspace storage is invalid');
      const result = executeWorkspace(dependencies, state, parsed.value);
      if (!result.ok || parsed.value.operation === 'workspace.read')
        return result;
      try {
        const written = await dependencies.storage.write({
          ownerId: identity.ownerId,
        ...
        if (!written.ok)
          return written.reason === 'conflict'
            ? conflict('Workspace changed before save')
            : unavailable('Workspace storage is unavailable');
      } catch {
        return unavailable('Workspace storage is unavailable');
      }
      return result;
