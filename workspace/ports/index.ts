import type { WorkspaceCommand, WorkspacePort, WorkspaceResult, WorkspaceState } from "../contracts/index.ts";
...
export interface WorkspaceAuthorizationPort {
  authorize(request: { operation: WorkspaceCommand["operation"]; permission: "workspace.read" | "workspace.write" | "workspace.publish"; identity: WorkspaceIdentity; ownerId: string }): Promise<{ allowed: true; ownerId: string } | { allowed: false; reason: string }>;
}

export interface OwnerWorkspaceStoragePort {
  read(request: { ownerId: string }): Promise<WorkspaceState | null>;
  write(request: { ownerId: string; expectedVersion: number; state: WorkspaceState }): Promise<{ ok: true } | { ok: false; reason: "conflict" | "unavailable" }>;
}
