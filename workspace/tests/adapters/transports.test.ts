...
test('HTTP MCP and CLI transports delegate the same command to their injected port', async () => {
  ...
  const port: WorkspacePort = {
    execute: async (command) => {
      ...
      return { ok: false, error: { code: 'denied', message: 'boundary' } };
    },
  };
  const http = createWorkspaceHttpHandler(port);
  const httpResponse = await http(
    new Request('https://example.test/api/guest/workspace', {
      method: 'POST',
      body: JSON.stringify({ operation: 'workspace.read', input: {} }),
    }),
  );
  assert.equal(httpResponse.status, 403);
  const mcp = await executeWorkspaceMcp(port, {
    operation: 'workspace.read',
    input: {},
  });
  const cli = await executeWorkspaceCli(port, [
