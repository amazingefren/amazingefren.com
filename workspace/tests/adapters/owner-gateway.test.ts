...
test('owner gateway fails closed when the private service is unconfigured or authentication is absent', async () => {
  const unavailable = await createOwnerWorkspaceGateway(undefined).read(
    new Request('https://example.test/workspace'),
  );
  assert.deepEqual(unavailable, {
    ok: false,
    error: {
      code: 'unavailable',
      message: 'Owner workspace service is unavailable',
    },
  });
  let calls = 0;
  const denied = await createOwnerWorkspaceGateway({
    fetch: async () => {
      calls += 1;
      return new Response();
    },
  }).read(new Request('https://example.test/workspace'));
  assert.deepEqual(denied, {
    ok: false,
    error: { code: 'denied', message: 'Owner authentication is required' },
  });
...
test('owner operation gateway rejects cross-origin and oversized commands before private service calls', async () => {
  let calls = 0;
  const gateway = createOwnerWorkspaceGateway({
    fetch: async () => {
      calls += 1;
      return new Response();
    },
  });
  const crossOrigin = await gateway.operation(
    new Request('https://example.test/api/workspace/operation', {
      method: 'POST',
  ...
  const large = await gateway.operation(
    new Request('https://example.test/api/workspace/operation', {
      method: 'POST',
      headers: {
  ...
  const streamed = await gateway.operation(
    new Request(
      'https://example.test/api/workspace/operation',
      Object.assign(
        {
          method: 'POST',
          headers: {
...
test('owner operation gateway denies a missing session before reading the command body', async () => {
  ...
  const response = await gateway.operation(
    new Request('https://example.test/api/workspace/operation', {
      method: 'POST',
...
test('owner operation gateway excludes guest reset before private service calls', async () => {
  ...
  const response = await gateway.operation(
    new Request('https://example.test/api/workspace/operation', {
      ...
      body: JSON.stringify({ operation: 'workspace.reset', input: {} }),
    }),
  );
...
test('owner gateway forwards only the session cookie and validated original command to the configured service', async () => {
  ...
  const gateway = createOwnerWorkspaceGateway({
    fetch: async (request) => {
      ...
      return new Response(JSON.stringify({ ok: true, value: state }), {
        status: 200,
  ...
  const response = await gateway.operation(
    new Request('https://example.test/api/workspace/operation', {
      ...
      headers: {
        origin: 'https://example.test',
        cookie: '__Host-ae-session=' + 'a'.repeat(43) + '; other=never-forward',
        'Cf-Access-Jwt-Assertion': 'never-forward',
      },
      body,
    }),
  );
  ...
  assert.equal(
    received[0].url,
    'https://workspace-owner.internal/api/workspace/operation',
  );
  assert.equal(received[0].headers.get('Cf-Access-Jwt-Assertion'), null);
  assert.equal(
    received[0].headers.get('cookie'),
...
test('owner gateway rejects a synthetic or malformed private service response', async () => {
  ...
  const gateway = createOwnerWorkspaceGateway({
    fetch: async () =>
      new Response(
        JSON.stringify({ ok: true, value: { ...synthetic, synthetic: true } }),
        { status: 200 },
      ),
  });
  const result = await gateway.read(
    new Request('https://example.test/workspace', {
      headers: { cookie: '__Host-ae-session=' + 'a'.repeat(43) },
    }),
  );
  assert.deepEqual(result, {
    ...
    error: {
      code: 'unavailable',
      message: 'Owner workspace service returned an invalid response',
    },
  });
});
...
test('owner gateway bounds upstream responses and error messages', async () => {
  ...
  const gateway = createOwnerWorkspaceGateway({
    fetch: async () =>
      Response.json({
        ok: false,
        error: { code: 'invalid', message: 'x'.repeat(5000) },
      }),
  });
  ...
  let cancelled = false;
  const oversized = createOwnerWorkspaceGateway({
    fetch: async () =>
      new Response(
        new ReadableStream({
          pull(controller) {
