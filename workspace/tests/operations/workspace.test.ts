...
test('workspace commands preserve exact revision snapshots when publishing', () => {
  ...
  const saved = executeWorkspaceCommand(
    initial.value,
    {
  ...
  if (!saved.ok) return;
  const published = executeWorkspaceCommand(
    saved.value,
    { operation: 'workspace.publish', input: { id: document.id, revision: 1 } },
    deps,
  );
  assert.equal(published.ok, true);
  if (!published.ok) return;
  assert.deepEqual(published.value.publications[0], {
  ...
  const conflicted = executeWorkspaceCommand(
    published.value,
    {
      operation: 'workspace.save-document',
      input: { id: document.id, title: 'Lost', body: 'Lost', revision: 1 },
    },
    deps,
  );
...
test('workspace restore creates a new draft revision and bounds revisions', () => {
  ...
  const saved = executeWorkspaceCommand(
    ...
    {
      operation: 'workspace.save-document',
      input: { id, title: 'Later', body: 'Later', revision: 1 },
    },
    deps,
  );
  assert.equal(saved.ok, true);
  if (!saved.ok) return;
  const restored = executeWorkspaceCommand(
    saved.value,
    {
      operation: 'workspace.restore-document',
      input: { id, sourceRevision: 1, revision: 2 },
    },
    deps,
  );
  ...
  assert.equal(
    restored.value.documents[0].body,
    'This guest workspace uses synthetic records.',
  );
});
...
test('workspace runtime parsing denies undeclared commands and extra fields', () => {
  assert.deepEqual(
    ...
    {
      ok: false,
      error: { code: 'invalid', message: 'This operation accepts no input' },
    },
  );
  assert.deepEqual(
    parseWorkspaceCommand({
      operation: 'workspace.create-task',
      input: { title: 'Task', hidden: true },
    }),
    { ok: false, error: { code: 'invalid', message: 'Task input is invalid' } },
  );
  assert.deepEqual(
    parseWorkspaceCommand({ operation: 'workspace.delete-all', input: {} }),
    {
      ok: false,
      error: { code: 'denied', message: 'Workspace operation is undeclared' },
    },
  );
  const result = executeWorkspaceCommand(
    seed(),
    {
      ...
      input: { id: 'missing', done: true },
    },
    dependencies(),
  );
  assert.deepEqual(result, {
    ok: false,
    error: { code: 'missing', message: 'Task was not found' },
  });
});
...
test('workspace link and publication operations reject missing records', () => {
  const state = seed();
  const deps = dependencies();
  const linked = executeWorkspaceCommand(
    state,
    {
      operation: 'workspace.link-documents',
      input: { from: state.documents[0].id, to: 'missing', label: 'depends' },
    },
    deps,
  );
  assert.equal(linked.ok, false);
  if (!linked.ok) assert.equal(linked.error.code, 'missing');
  const withdrawn = executeWorkspaceCommand(
    state,
    { operation: 'workspace.withdraw', input: { id: state.documents[0].id } },
    deps,
  );
