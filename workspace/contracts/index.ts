...
export type WorkspaceExperiment = { id: string; title: string; hypothesis: string; status: 'planned' | 'running' | 'complete'; observations: string };
export type WorkspaceRelationship = { id: string; from: string; to: string; label: string };
export type WorkspacePublication = { documentId: string; title: string; body: string; revision: number; publishedAt: string };
export type WorkspaceActivity = { id: string; label: string; at: string };
export type WorkspaceState = { version: number; synthetic: boolean; documents: WorkspaceDocument[]; tasks: WorkspaceTask[]; experiments: WorkspaceExperiment[]; relationships: WorkspaceRelationship[]; publications: WorkspacePublication[]; activity: WorkspaceActivity[] };
export type WorkspaceError = { code: 'invalid' | 'denied' | 'missing' | 'conflict' | 'unavailable'; message: string };
...
export type WorkspaceCommand =
  ...
  | { operation: 'workspace.create-document'; input: { title: string; body?: string } }
  | { operation: 'workspace.save-document'; input: { id: string; title: string; body: string; revision: number } }
  | { operation: 'workspace.restore-document'; input: { id: string; sourceRevision: number; revision: number } }
  | { operation: 'workspace.create-task'; input: { title: string; documentId?: string } }
  | { operation: 'workspace.complete-task'; input: { id: string; done: boolean } }
  | { operation: 'workspace.create-experiment'; input: { title: string; hypothesis: string } }
  | { operation: 'workspace.update-experiment'; input: { id: string; status: WorkspaceExperiment['status']; observations: string } }
  | { operation: 'workspace.link-documents'; input: { from: string; to: string; label: string } }
