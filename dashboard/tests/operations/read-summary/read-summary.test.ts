...
import { readSummary, type ReadSummaryDependencies } from "../../../operations/read-summary/index.ts";
...
const input = { from: "2026-09-01T00:00:00.000Z", to: "2026-09-02T00:00:00.000Z" };
...
function snapshot(): TelemetrySnapshot {
  return {
    ...
    metrics: {
      "page-views": { status: "available", numerator: 12, denominator: null, source: "consented-audience-aggregates" },
      "honeypot-triggers": { status: "stale", numerator: 3, denominator: null, source: "operational-security-aggregates" },
      "honeypot-trigger-rate": { status: "available", numerator: 3, denominator: 12, source: "operational-security-aggregates" }
    }
  };
}
...
test("owner summary denies a foreign owner before telemetry reads", async () => {
  ...
  const result = await readSummary(
    {
      now: () => 0,
      auth: { authorize: async () => { authReads += 1; return { allowed: true, ownerId: "owner-2" }; } },
      telemetry: { read: async () => { telemetryReads += 1; return { kind: "snapshot", snapshot: snapshot() }; } }
    },
    { input, caller, ownerId: "owner-2" }
  );

  assert.deepEqual(result, { ok: false, error: { code: "forbidden", message: "The caller cannot read another owner" } });
...
test("owner summary denies guest credentials before protected reads", async () => {
  ...
  const result = await readSummary(
    {
      now: () => 0,
      auth: { authorize: async () => { authReads += 1; return { allowed: true, ownerId: "owner-1" }; } },
      telemetry: { read: async () => { telemetryReads += 1; return { kind: "snapshot", snapshot: snapshot() }; } }
    },
...
test("owner summary returns authorization denial before telemetry reads", async () => {
  ...
  const result = await readSummary(
    {
      now: () => 0,
      auth: { authorize: async () => ({ allowed: false, error: { code: "forbidden", message: "dashboard.read is required" } }) },
      telemetry: { read: async () => { telemetryReads += 1; return { kind: "snapshot", snapshot: snapshot() }; } }
    },
    { input, caller }
  );

  assert.deepEqual(result, { ok: false, error: { code: "forbidden", message: "dashboard.read is required" } });
  assert.equal(telemetryReads, 0);
});
...
test("owner summary fails closed when authorization is unavailable", async () => {
  ...
  const result = await readSummary(
    {
      now: () => 0,
      auth: { authorize: async () => { throw new Error("auth down"); } },
      telemetry: { read: async () => { telemetryReads += 1; return { kind: "snapshot", snapshot: snapshot() }; } }
    },
    { input, caller }
  );

  assert.deepEqual(result, { ok: false, error: { code: "unavailable", message: "Authorization is unavailable" } });
  assert.equal(telemetryReads, 0);
});
...
test("owner summary rejects reversed, excessive, and non-finite windows before reads", async (t) => {
  ...
  const dependencies: ReadSummaryDependencies = {
    now: () => 0,
    auth: { authorize: async () => { reads += 1; return { allowed: true, ownerId: "owner-1" }; } },
    telemetry: { read: async () => { reads += 1; return { kind: "snapshot", snapshot: snapshot() }; } }
  };
  const cases = [
  ...
  const unavailable = await readSummary(
    {
      ...
      telemetry: { read: async () => ({ kind: "unavailable", source: "collector-gap", observedAt: null }) }
    },
    { input, caller }
  );
  assert.equal(unavailable.ok, true);
  if (unavailable.ok) {
    assert.equal(unavailable.value.metrics.every((metric) => metric.value === null), true);
    assert.equal(unavailable.value.metrics.every((metric) => metric.status === "unavailable"), true);
  ...
  const failed = await readSummary(
    {
      now: () => 0,
      auth: { authorize: async () => ({ allowed: true, ownerId: "owner-1" }) },
      telemetry: { read: async () => { throw new Error("collector down"); } }
    },
    { input, caller }
  );
  assert.equal(failed.ok, true);
  if (failed.ok) assert.equal(failed.value.metrics[0].source, "telemetry-unavailable");
});
...
test("owner summary accepts leap days and equivalent offset windows", async () => {
  ...
  }, { input: { from: "2024-02-29T02:00:00+02:00", to: "2024-03-01T00:00:00Z" }, caller });
  assert.equal(result.ok, true);
});
