...
test('guest HTTP adapter normalizes datetime-local input and returns the shared summary', async () => {
  const handler = createGuestSummaryHttpHandler(
    { now: () => 0, telemetry: createSyntheticGuestTelemetry() },
    'guest-preview',
  );
  const response = await handler(
    new Request(`https://example.test/api/guest/dashboard/summary${query}`),
  );
  const summary = (await response.json()) as {
    dataScope: string;
    from: string;
    metrics: Array<{ value: number | null }>;
  };
...
test('HTTP adapters reject invalid input and methods', async () => {
  ...
  const invalid = await handler(
    new Request('https://example.test/api/guest/dashboard/summary'),
  );
  ...
  const unknown = await handler(
    new Request(
      `https://example.test/api/guest/dashboard/summary${query}&extra=1`,
    ),
  );
  const duplicate = await handler(
    new Request(
      `https://example.test/api/guest/dashboard/summary${query}&from=2026-09-01T01%3A00`,
    ),
  );
...
test('owner HTTP adapter denies an anonymous caller before owner telemetry', async () => {
  ...
  const handler = createOwnerSummaryHttpHandler(
    {
      ...
      telemetry: {
        read: async () => {
          reads += 1;
          return { kind: 'unavailable', source: 'owner', observedAt: null };
        },
      },
    },
    () => null,
  ...
  const response = await handler(
    new Request(`https://example.test/api/dashboard/summary${query}`),
  );
