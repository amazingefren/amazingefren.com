...
import { parseSummaryCliArgs, readGuestSummaryCli } from "../../../adapters/cli/summary.ts";
import { createSyntheticGuestTelemetry } from "../../../ports/telemetry/synthetic.ts";

test("CLI adapter parses bounded UTC window flags", () => {
  const parsed = parseSummaryCliArgs(["--from", "2026-09-01T00:00", "--to", "2026-09-02T00:00"]);
  assert.deepEqual(parsed, { ok: true, value: { from: "2026-09-01T00:00:00Z", to: "2026-09-02T00:00:00Z" } });
  const invalid = parseSummaryCliArgs(["--from", "2026-09-01T00:00", "--bad", "value"]);
...
test("CLI rejects repeated window flags", () => {
  const result = parseSummaryCliArgs(["--from", "2026-09-01T00:00", "--from", "2026-09-02T00:00", "--to", "2026-09-03T00:00"]);
