...
import { metricEvidenceLabel, metricStatusLabel, metricValueLabel, localDateTimeValue } from "../../../ui/summary/index.ts";
...
test("guest dashboard presentation keeps measured counts and rate evidence readable", () => {
  ...
  assert.equal(metricValueLabel({ ...availableCount, id: "honeypot-trigger-rate", value: 0.125, unit: "ratio", numerator: 1, denominator: 8 }), "12.5%");
  assert.equal(metricEvidenceLabel({ ...availableCount, id: "honeypot-trigger-rate", value: 0.125, unit: "ratio", numerator: 1, denominator: 8 }), "1 triggered / 8 eligible evaluations");
});
...
test("guest dashboard presentation preserves unavailable and local form states", () => {
  ...
  assert.equal(metricEvidenceLabel(undefined), "No measured value in this window");
  assert.equal(metricStatusLabel("suppressed"), "Suppressed");
  assert.equal(metricStatusLabel(undefined), "Unavailable");
  assert.equal(localDateTimeValue("2026-09-01T12:30:00.000Z"), "2026-09-01T12:30");
