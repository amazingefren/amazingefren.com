import type { DashboardCaller, DashboardAuthPort } from "../../ports/auth/index.ts";
import type { DashboardResult, DashboardSummary } from "../../contracts/summary.ts";
import type { GuestTelemetryPort, OwnerTelemetryPort } from "../../ports/telemetry/index.ts";
...
export function parseSummaryCliArgs(args: readonly string[]) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (flag !== "--from" && flag !== "--to") return { ok: false as const, error: { code: "invalid_input" as const, message: `Unknown option: ${flag}` } };
    if (flag.slice(2) in input) return { ok: false as const, error: { code: "invalid_input" as const, message: `${flag} must be provided once` } };
    const value = args[index + 1];
    if (!value || value.startsWith("--")) return { ok: false as const, error: { code: "invalid_input" as const, message: `${flag} requires a value` } };
...
export async function readSummaryCli(
  ...
  if (!parsed.ok) return parsed;
  return readSummary(
    { now: dependencies.now, auth: dependencies.auth, telemetry: dependencies.ownerTelemetry },
