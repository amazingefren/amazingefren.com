...
export function createOwnerSummaryHttpHandler(
  ...
  resolveCaller: (
    request: Request,
  ) => DashboardCaller | null | Promise<DashboardCaller | null>,
): (request: Request) => Promise<Response> {
  return async (request) => {
    if (request.method !== 'GET')
      return errorResponse(
    ...
    const command: ReadSummaryCommand = {
      input: input.value,
      caller: caller ?? unauthenticatedCaller(),
    };
