...
export function DashboardChart({ metrics }: DashboardChartProps) {
  ...
  const countValues = DASHBOARD_METRIC_IDS.map((id) => metricById.get(id))
    .filter(isAvailableCount)
    .map((metric) => metric.value);
  const countMax = Math.max(...countValues, 1);

  return (
    <section
      className="dashboard-panel dashboard-signals"
      aria-labelledby="dashboard-signals-title"
    >
      <div className="dashboard-panel-heading">
        ...
        <p className="dashboard-panel-note">
          Bar lengths use each metric&apos;s own unit. The table carries exact
          values.
        </p>
      </div>
          ...
          {DASHBOARD_METRIC_IDS.map((id) => {
            ...
            return (
              <div className="dashboard-bar-row" key={id}>
                ...
                <div
                  className={`dashboard-bar-track dashboard-bar-${metric?.status ?? 'unavailable'}`}
                >
      ...
      <div className="dashboard-table-wrap">
        <table className="dashboard-table">
          <caption className="dashboard-visually-hidden">
...
function MetricTableRow({
  ...
  return (
    <tr>
      <th scope="row">
        <span>{metricLabel(id)}</span>
        <small>{metricDescription(id)}</small>
      </th>
      <td>
        <strong>{metricValueLabel(metric)}</strong>
        <small>{metricStatusLabel(metric?.status)}</small>
      </td>
...
function chartWidth(
  ...
  if (metric.unit === 'ratio')
    return Math.min(Math.max(metric.value, 0), 1) * 100;
...
function chartLabel(metrics: readonly DashboardMetric[]): string {
  return metrics
    .map(
      (metric) =>
        `${metricLabel(metric.id)}: ${metricValueLabel(metric)}; ${metricStatusLabel(metric.status)}`,
    )
    .join('. ');
}

function isAvailableCount(
  metric: DashboardMetric | undefined,
): metric is DashboardMetric & { value: number } {
  return (
    metric !== undefined && metric.unit === 'count' && metric.value !== null
  );
}
