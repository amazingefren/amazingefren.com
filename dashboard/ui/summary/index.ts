import type { DashboardMetric, DashboardMetricId, DashboardMetricStatus } from "../../contracts/summary.ts";
...
export const DASHBOARD_METRIC_DESCRIPTIONS: Record<DashboardMetricId, string> = {
  ...
  "honeypot-trigger-rate": "Triggered eligible evaluations divided by all eligible evaluations."
};
...
export function metricStatusLabel(status: DashboardMetricStatus | undefined): string {
...
export function metricEvidenceLabel(metric: DashboardMetric | undefined): string {
  if (!metric || metric.value === null) return "No measured value in this window";
