...
export type SummaryValidation =
  { ok: true; window: DashboardWindow } | { ok: false; error: DashboardError };

export function validateSummaryInput(input: unknown): SummaryValidation {
  if (!isSummaryInput(input)) {
    return {
      ok: false,
      error: invalidInput('from and to must be date-time strings'),
    };
  }
  ...
  if (!Number.isFinite(fromMs) || !Number.isFinite(toMs)) {
    return {
      ok: false,
      error: invalidInput('from and to must be finite timestamps'),
    };
  }
  if (fromMs >= toMs) {
  ...
  if (toMs - fromMs > MAX_QUERY_WINDOW_MS) {
    return {
      ok: false,
      error: invalidInput(
        'the requested window exceeds the maximum of 31 days',
      ),
    };
  }
...
export function buildSummary(
  ...
  return {
    ...
    to: input.to,
    generatedAt,
    freshness: validDateTimeOrNull(freshness),
    metrics,
  };
}
...
function toMetric(
  ...
  const unit: DashboardMetricUnit =
    id === 'honeypot-trigger-rate' ? 'ratio' : 'count';
  const source =
    dataScope === 'synthetic'
      ? 'synthetic-fixture'
      : sourceName(observation?.source, fallbackSource);
  if (
    !observation ||
...
function sourceName(
  metricSource: string | undefined,
  fallbackSource: string,
): string {
  if (typeof metricSource === 'string' && metricSource.trim().length > 0)
    return metricSource;
...
function parseTimestamp(value: string): number {
  const match =
    /^(\d{4})-(\d{2})-(\d{2})T([01]\d|2[0-3]):([0-5]\d):([0-5]\d)(?:\.\d+)?(?:Z|[+-](?:[01]\d|2[0-3]):[0-5]\d)$/i.exec(
      value,
    );
  if (!match) return NaN;
