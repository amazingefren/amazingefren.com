...
export default {
  ...
  copy: {
    ...
    forbiddenUiPhrases: ['Tracked System Scope', 'Accepted items show verified tracked work. They do not certify a whole system.', 'A place for longer thoughts on software, applied AI, and the work around them.', 'Original essays and stable copies will appear here as they are ready.', 'A little bit of everything.', 'All in one place.', 'A look inside the tools, projects, and processes', 'Each agent has a task, an owned area, and a handoff.', 'Select a system to look inside', 'frozen workspace contract', 'What active would mean', 'Role control', 'Server supplied only', 'Approved catalog', 'Allowlisted data', 'External boundaries', 'Session boundary', 'Saving never publishes.', 'These are manifest metadata.']
  },
  interaction: {
    rules: [
  ...
  behaviors: [
    ...
    { id: 'design.motion', status: 'approved', purpose: 'Keep motion optional.', implementation: 'design/ui/primitives.css', rules: ['Respect prefers-reduced-motion.', 'No new autoplay motion, flicker, or attention loops except the owner-approved Open field motion: a single bird flight/lock sequence and user-triggered public navigation transitions, both with reduced-motion support.', 'Static CRT texture belongs only to the approved research artwork.'] }
  ],
  components: [
    { id: 'design.owner-access', status: 'approved', purpose: 'Open the single owner workspace with a passkey.', implementation: 'auth/ui/Access.tsx', rules: ['Owner approved ae-workbench/auth-launch on 2026-09-10. Use shared BrandMark, ThemeControl, tokens, native setup disclosure, and visible pending/error states.', 'Keep /auth/me unlinked from public navigation. Setup takes a one-time secret; sign-in and later enrollment require passkeys.'] },
  ...
  patterns: [
    { id: 'design.public-navigation-transition', status: 'approved', purpose: 'Soften changes between public pages.', implementation: 'web/ui/shared/public-transitions.css', rules: ['Owner approved the slower prototype on 2026-09-09.', 'Animate only content: 180ms exit, 400ms entrance after 60ms, with 4px upward settle.', 'Show only one static root, header, and footer snapshot to avoid contrast changes.', 'Use native document navigation. Reduced motion and unsupported browsers keep normal links and history. Workspace pages do not opt in.'] },
    { id: 'design.work-field', status: 'approved', purpose: 'Organize human and agent work by next action and dependency readiness.', implementation: null, rules: ['Owner authorized application implementation on 2026-09-09. Three-item focus, flat work rows, cross-space lenses, connected item details, and tracked system scope.', 'Use shared tokens and native controls. No draggable columns, canvas gestures, custom dialogs, or new shortcut engine.', 'Review both themes, narrow panes, keyboard return focus, empty states, unsaved input, and blocked actions before adoption.'] },
    { id: 'design.writing-workflows', status: 'approved', purpose: 'Separate fast personal capture from focused publication writing.', implementation: 'workspace/ui/writing/index.ts', rules: ['Owner approved the revised writing prototype for application implementation on 2026-09-09.', 'Notes use a quick capture field and compact entries. Publications open from a library into one centered manuscript.', 'Hide the workspace sidebar and header during publication writing. Keep save status and exit available; reveal settings, chapters, images, and publishing controls on request.', 'Reuse AE Design typography, tokens, theme control, native dialogs, and visible keyboard focus. Vim editing is remappable and optional.'] },
    { id: 'design.kanban', status: 'proposed', purpose: 'Track cards across named boards and review stages.', implementation: null, rules: ['Private prototype: ae-workbench/kanban/prototype.manifest.ts. Reuse workspace tokens, native forms, inline details, and shared theme controls.', 'Use board selection, named columns, text blockers, and a keyboard-accessible move select. Stack columns on mobile. Review empty, invalid, conflict, unavailable, and saved states before adoption.', 'Built requires evidence and owner review. Sample placements are not implementation findings.'] },
    { id: 'design.observation-background-study', status: 'approved', purpose: 'Present the approved Open field home.', implementation: 'web/ui/landing/OpenField.tsx', rules: ['Owner approved Open field and optional animation on 2026-09-09. Center the introduction and contain the scene between header and footer.', 'Light uses the approved bird paths and one corner-bracket detection box on the large middle bird, with no crosshairs. Dark uses the approved galaxy. Artwork opacity and alpha masks keep reading areas clear.', 'Birds enter together once, using static silhouettes. Only the large middle bird receives a box, easing into a stop as detection locks. Dark mode uses the original static galaxy image. No motion button. Reduced motion and no JavaScript show static artwork. Pause when hidden. Never delay content or simulate live telemetry.'] },
