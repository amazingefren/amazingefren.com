...
export function installTheme(document: Document, storage: Pick<Storage, 'getItem' | 'setItem' | 'removeItem'> | null, key: string) {
  function normalize(value: unknown) { return value === 'light' || value === 'dark' ? value : 'system'; }
  let current = 'system';
  try { current = normalize(storage?.getItem(key)); } catch {}
  ...
  apply(current);
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => apply(current));
  new MutationObserver(() => apply(current)).observe(document.body ?? document.documentElement, { childList: true, subtree: true });
}
