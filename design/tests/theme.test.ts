...
function fixture(saved: string | null, blocked = false) {
  ...
  const buttons = ['system', 'light', 'dark'].map(choice => {
    ...
    return { getAttribute: (key: string) => values.get(key), setAttribute: (key: string, value: string) => values.set(key, value), classList: { toggle() {} }, closest() { return this; } };
  });
  let observe = () => {};
  const document = {
    readyState: 'loading',
  ...
  const storage = {
    ...
    removeItem() { if (blocked) throw Error('blocked'); saved = null; }
  };
  runInNewContext(themeBootstrap(), { document, window: { get localStorage() { if (blocked) throw Error('blocked'); return storage; } }, MutationObserver: class { constructor(callback: () => void) { observe = callback; } observe() {} } });
  return {
    theme: () => attributes.get('data-theme'),
    saved: () => saved,
    selected: () => buttons.filter(button => button.getAttribute('aria-pressed') === 'true').map(button => button.getAttribute('data-theme-choice')),
    click: (index: number) => (listeners.get('click') as unknown as (event: unknown) => void)({ target: buttons[index] }),
    replaceContent: () => { buttons.forEach(button => button.setAttribute('aria-pressed', 'false')); observe(); }
  };
}
