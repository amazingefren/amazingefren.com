...
export function inspectStyles(source: string, canonical = false): string[] {
  ...
  for (const declaration of source.matchAll(/(?:^|[;{])\s*(?:color|background(?:-color)?|border(?:-(?:top|right|bottom|left))?-color|outline-color|fill|stroke)\s*:\s*([^;}]+)/g)) {
    const value = declaration[1].replace(/\s*!important\s*$/, '').trim();
    if (/^[a-z-]+$/i.test(value) && !/^(transparent|currentcolor|inherit|initial|unset|revert|revert-layer|none|auto)$/i.test(value)) errors.push('Raw color');
  }
  ...
  for (const rule of source.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    ...
    for (const token of design.interaction.forbiddenHoverTokens) {
      if (rule[2].includes(`var(${token})`)) errors.push(`Wrong hover token ${token}`);
    }
  }
...
export function inspectCopy(source: string): string[] {
  ...
  return design.copy.forbiddenUiPhrases.filter(phrase => normalized.includes(phrase.toLowerCase())).map(phrase => `Prohibited UI copy: ${phrase}`);
}
...
export async function checkDesign(root: URL, systems: readonly { id: string; dependencies: readonly string[] }[]) {
  ...
  for (const theme of [lightTheme, darkTheme]) for (const pair of design.interaction.contrastPairs) {
    const tokens = theme as Record<string, string>;
    assert(contrast(tokens[pair.foreground], tokens[pair.background]) >= pair.minimum, `Insufficient contrast: ${pair.foreground} on ${pair.background}`);
  }
  ...
  async function scan(path: string) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) await scan(child);
        ...
        const errors = [...inspectStyles(source), ...(child.endsWith('.tsx') ? inspectCopy(source) : [])];
        assert.equal(errors.length, 0, `${child}: ${errors.join(', ')}`);
        count++;
      }
    }
  }
  for (const path of [...systems.filter(system => system.id !== 'design').map(system => `${system.id}/ui`), 'web/adapters/http', 'web/composition', 'design/ui']) await scan(path);
  return count;
}
