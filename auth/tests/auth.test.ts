...
function setup() {
  ...
  const dependencies = {
    ...
    crypto: authCrypto,
    passkeys: createPasskeys('https://example.com'),
    now: () => now,
...
async function enroll(context: ReturnType<typeof setup>) {
  ...
  const options = await executeAuth(
  ...
  const result = await executeAuth(
    'registration-verify',
    { response },
    { session: null, challenge: options.challengeToken! },
    dependencies,
  );
...
test('bootstrap requires secret and remains disabled after deleting every passkey', async () => {
  ...
  assert.equal(
    (
      await executeAuth(
        'registration-options',
        { bootstrapToken: context.dependencies.bootstrapToken },
        noCookies,
        context.dependencies,
      )
    ).status,
...
test('two bootstrap ceremonies can only create one owner credential', async () => {
  ...
  const first = await executeAuth(
    'registration-options',
    input,
    noCookies,
    context.dependencies,
  );
  const second = await executeAuth(
    'registration-options',
    input,
    noCookies,
    context.dependencies,
  ...
  const results = await Promise.all(
    [first, second].map((options, i) =>
      executeAuth(
        'registration-verify',
        {
          response: (i ? device2 : context.device).registration(
  ...
  assert.equal(
    context.database.sql
      .prepare('SELECT count(*) AS n FROM auth_credentials')
      .get()!.n,
    1,
  );
});
test('challenge cookie binding, expiry, single use, and invalid verification consume attempts', async () => {
  ...
  const response = context.device.authentication(
    (options.body as { challenge: string }).challenge,
  );
  assert.equal(
    (
  ...
  assert.deepEqual(results.map((result) => result.status).sort(), [200, 403]);
  const expired = await executeAuth(
    'authentication-options',
    {},
    noCookies,
    context.dependencies,
  ...
  assert.equal(
    (
      await executeAuth(
        'authentication-verify',
        { response },
        { session: null, challenge: expired.challengeToken! },
        context.dependencies,
      )
    ).status,
    403,
  );
  const invalid = await executeAuth(
    'authentication-options',
    {},
    noCookies,
    context.dependencies,
  ...
  assert.equal(
    (
      await executeAuth(
        'authentication-verify',
        { response: {} },
        { session: null, challenge: invalid.challengeToken! },
        context.dependencies,
      )
    ).status,
    403,
  ...
  assert.equal(
    await context.dependencies.store.consumeChallenge(
      await authCrypto.hash(invalid.challengeToken!),
      'authentication',
...
test('session stores only hashes, rejects forged or duplicate cookies, expires, and logs out', async () => {
  ...
  const request = (cookie: string) =>
    new Request('https://example.com/workspace', {
      headers: { cookie, 'cf-access-jwt-assertion': 'forged' },
    });
  const row = context.database.sql
    .prepare('SELECT token_hash, expires_at, verified_at FROM auth_sessions')
    .get()!;
  assert.notEqual(row.token_hash, token);
  assert.equal(
    Number(row.expires_at) - Number(row.verified_at),
    sessionLifetime,
  );
  assert.equal(
    (
      await readOwnerSession(
        request(`__Host-ae-session=${token}`),
        context.database,
      )
    )?.ownerId,
    ownerId,
  ...
  assert.equal(
    await readOwnerSession(
      request(`__Host-ae-session=${'X'.repeat(43)}`),
      context.database,
    ),
    null,
  );
  assert.equal(
    await readOwnerSession(
      request(`__Host-ae-session=${token}; __Host-ae-session=${token}`),
      context.database,
    ),
    null,
  );
  assert.equal(
    await readOwnerSession(request(`__Host-ae-session=${token}`)),
    null,
  );
  await executeAuth(
    'logout',
    {},
    { session: token, challenge: null },
    context.dependencies,
  );
  assert.equal(
    await readOwnerSession(
  ...
  assert.equal(
    await readOwnerSession(
      request(`__Host-ae-session=${token2}`),
      context.database,
    ),
    null,
  );
...
async function enrollExisting(context: ReturnType<typeof setup>) {
  ...
  const result = await executeAuth(
    'authentication-verify',
    {
      response: context.device.authentication(
        (options.body as { challenge: string }).challenge,
      ),
    },
    { session: null, challenge: options.challengeToken! },
  ...
  assert.equal(
    (
      await executeAuth(
        ...
        { session: token, challenge: options.challengeToken! },
        context.dependencies,
      )
    ).status,
    403,
  ...
  assert.equal(
    (
      await executeAuth(
        'registration-options',
        {},
        { session: replacement, challenge: null },
        context.dependencies,
      )
    ).status,
    403,
...
test('HTTP rejects cross-site, missing origin, bad method, unknown routes, and oversized bodies', async () => {
  ...
  const make = (
    ...
    new Request(`${dependencies.origin}/api/auth/${path}`, {
      method,
      headers: { origin, 'content-type': 'application/json' },
      ...(method === 'POST' ? { body } : {}),
    });
  for (const request of [make('https://evil.example'), make('')])
    assert.equal((await handleAuthHttp(request, dependencies)).status, 403);
  const methodDenied = await handleAuthHttp(
    make(dependencies.origin, '{}', 'GET'),
    dependencies,
  );
  ...
  assert.equal(
    (
      await handleAuthHttp(
        make(dependencies.origin, JSON.stringify({ role: 'owner' })),
        dependencies,
      )
    ).status,
    400,
  ...
  assert.equal(
    (
      await handleAuthHttp(
        make(dependencies.origin, '{}', 'POST', 'undeclared'),
        dependencies,
      )
    ).status,
    404,
  ...
  assert.equal(
    (
      await handleAuthHttp(
        make(dependencies.origin, ' '.repeat(16385)),
        dependencies,
      )
    ).status,
    400,
  ...
  assert.match(
    ...
    /__Host-ae-challenge=[A-Za-z0-9_-]{43}; Path=\/; Secure; HttpOnly; SameSite=Strict; Max-Age=300/,
  );
  assert.equal(
    readCookie(result.headers.get('set-cookie'), '__Host-ae-challenge')?.length,
    43,
  );
});
test('attempt and challenge storage enforce limits and clean expired state', async () => {
  ...
  for (let i = 0; i < 20; i++)
    assert.equal(
      await dependencies.store.allowAttempt('ip', dependencies.now()),
      true,
    );
  assert.equal(
    await dependencies.store.allowAttempt('ip', dependencies.now()),
  ...
  const insert = database.sql.prepare(
    "INSERT INTO auth_challenges VALUES (?, 'challenge', 'authentication', 'public', NULL, ?)",
  );
  for (let i = 0; i < 1024; i++)
    insert.run(String(i), dependencies.now() + 300000);
  assert.equal(
    (await executeAuth('authentication-options', {}, noCookies, dependencies))
      .status,
    429,
  );
  advance(300001);
  ...
  assert.equal(
    database.sql.prepare('SELECT count(*) AS n FROM auth_challenges').get()!.n,
    1,
  );
});
test('backup passkey enrollment succeeds and its session works independently', async () => {
  ...
  const options = await executeAuth(
    'registration-options',
    {},
    { session: original, challenge: null },
    context.dependencies,
  );
  ...
  const result = await executeAuth(
    'registration-verify',
    {
      response: backup.registration(
        (options.body as { challenge: string }).challenge,
      ),
    },
    { session: original, challenge: options.challengeToken! },
    context.dependencies,
  );
  assert.equal(result.status, 200);
  assert.equal(
  ...
  assert.equal(
    await context.dependencies.store.session(
      await authCrypto.hash(original),
      context.dependencies.now(),
    ),
    null,
  );
  const challenge = await executeAuth(
    'authentication-options',
    {},
    noCookies,
    context.dependencies,
  );
  const signedIn = await executeAuth(
    'authentication-verify',
    {
      response: backup.authentication(
        (challenge.body as { challenge: string }).challenge,
      ),
    },
    { session: null, challenge: challenge.challengeToken! },
    context.dependencies,
  );
...
test('credential persistence rejects revoked owner authorization even after earlier validation', async () => {
  ...
  const challenge = {
    ...
    sessionHash: hash,
    expiresAt: context.dependencies.now() + 300000,
  };
  assert.ok(
    await context.dependencies.store.session(hash, context.dependencies.now()),
  );
  await context.dependencies.store.deleteSession(hash);
  assert.equal(
    await context.dependencies.store.saveCredential(
