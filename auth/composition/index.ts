...
export async function handleAuthRequest(
  ...
  if (!environment.AUTH_DB || !validOrigin(environment.AUTH_ORIGIN))
    return authResponse({ error: 'unavailable' }, 503);
  return handleAuthHttp(request, {
    store: createAuthStore(environment.AUTH_DB),
    crypto: authCrypto,
    passkeys: createPasskeys(environment.AUTH_ORIGIN),
    now: Date.now,
    origin: environment.AUTH_ORIGIN,
    bootstrapToken: environment.AUTH_BOOTSTRAP_TOKEN,
  });
}
