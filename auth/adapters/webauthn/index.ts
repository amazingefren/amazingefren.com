...
function validClientContext(response: unknown): boolean {
  ...
  if (
    ...
    !('clientDataJSON' in payload) ||
    typeof payload.clientDataJSON !== 'string'
  )
    return false;
  const client: unknown = JSON.parse(
    new TextDecoder().decode(decode(payload.clientDataJSON)),
  );
  if (!client || typeof client !== 'object') return false;
  return (
    (!('crossOrigin' in client) || client.crossOrigin === false) &&
    !('topOrigin' in client)
  );
}
export function createPasskeys(origin: string): Passkeys {
  ...
  return {
    registrationOptions: (credentials) =>
      generateRegistrationOptions({
        rpName: 'AE Workspace',
        rpID,
    ...
    async verifyAuthentication(response, challenge, credential) {
      try {
        ...
        if (
          ...
          assertion.response.userHandle !==
            encode(new TextEncoder().encode(ownerId))
        )
          return null;
