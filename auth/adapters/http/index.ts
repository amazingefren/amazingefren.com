...
import {
  executeAuth,
  type AuthOperation,
} from '../../operations/passkeys/index.ts';
...
export async function handleAuthHttp(
  ...
  if (!path.startsWith('/api/auth/') || !operations.has(operation))
    return authResponse({ error: 'not_found' }, 404);
  ...
  if (
    !validOrigin(dependencies.origin) ||
    ...
    return authResponse({ error: 'forbidden' }, 403);
  try {
    const key = await dependencies.crypto.hash(
      request.headers.get('cf-connecting-ip') ?? 'unidentified',
    );
    if (!(await dependencies.store.allowAttempt(key, dependencies.now())))
      return authResponse({ error: 'rate_limited' }, 429);
    const input = await readInput(request);
    if (!input) return authResponse({ error: 'invalid_request' }, 400);
    const allowedKeys =
      operation === 'registration-options'
        ? ['bootstrapToken']
        : operation.endsWith('-verify')
          ? ['response']
          : [];
    if (Object.keys(input).some((key) => !allowedKeys.includes(key)))
      return authResponse({ error: 'invalid_request' }, 400);
    if (
      operation.endsWith('-verify') &&
      (!input.response ||
      ...
      return authResponse({ error: 'invalid_request' }, 400);
    if (
      input.bootstrapToken !== undefined &&
    ...
    const result = await executeAuth(
      ...
      input,
      {
        session: readCookie(request.headers.get('cookie'), sessionCookie),
        challenge: readCookie(request.headers.get('cookie'), challengeCookie),
      },
      dependencies,
    );
    ...
    if (result.sessionToken)
      cookies.push(
        writeCookie(sessionCookie, result.sessionToken, sessionLifetime / 1000),
      );
