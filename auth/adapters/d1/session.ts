...
export async function readOwnerSession(
  ...
  try {
    ...
    const row = await database
      .prepare(
        'SELECT owner_id AS ownerId, verified_at AS verifiedAt, expires_at AS expiresAt FROM auth_sessions WHERE token_hash = ? AND expires_at > ? AND verified_at <= ?',
      )
      .bind(await hashToken(token), now, now)
      .first<OwnerSession>();
