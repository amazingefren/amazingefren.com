...
const invalid = (): Result => ({ ok: false, error: 'invalid', message: 'Invalid work command.' });
...
async function dispatch(port: WorkPort, value: unknown): Promise<Result> {
  ...
  try { return await port.execute(command); }
  catch { return { ok: false, error: 'unavailable', message: 'Work is unavailable. Retry.' }; }
}
export function createWorkHttp(port: WorkPort) {
  return async (request: Request): Promise<Response> => {
    if (request.method !== 'POST') return Response.json(invalid(), { status: 405, headers: { Allow: 'POST', 'Cache-Control': 'no-store' } });
    let result: Result;
    try {
      ...
      if (reader) {
        while (true) {
          ...
          if (size > limit) { await reader.cancel(); throw Error('limit'); }
          chunks.push(part.value);
        }
      }
      const bytes = new Uint8Array(size); let offset = 0;
      for (const chunk of chunks) { bytes.set(chunk, offset); offset += chunk.length; }
      result = await dispatch(port, JSON.parse(new TextDecoder().decode(bytes)));
    } catch { result = invalid(); }
    const status = result.ok ? 200 : { invalid: 400, denied: 403, conflict: 409, unavailable: 503 }[result.error];
...
export function createWorkCli(port: WorkPort) {
  return (input: string): Promise<Result> => {
    ...
    try { return dispatch(port, JSON.parse(input)); }
    catch { return Promise.resolve(invalid()); }
  };
}
