  ...
  return {
    async operation(request: Request): Promise<Response> {
      if (request.method !== 'POST')
        return json({ ok: false, error: 'invalid', message: 'Use POST.' }, 405);
      if (request.headers.get('origin') !== new URL(request.url).origin)
        return json(
          { ok: false, error: 'denied', message: 'Sign in to continue.' },
          403,
        );
      ...
      if (
        !command ||
        command.operation !== new URL(request.url).pathname.split('/').at(-1)
      )
        return json(
          { ok: false, error: 'invalid', message: 'Invalid work request.' },
          400,
        );
      try {
        const response = await service.fetch(
          new Request('https://workspace-owner.internal/api/work/operation', {
            method: 'POST',
            headers: { 'content-type': 'application/json', cookie: assertion },
...
function json(result: Result, status: number) {
  ...
  return json(
    {
      ok: false,
      error: 'unavailable',
      message: 'Work is unavailable. Try again.',
    },
    503,
  );
}
